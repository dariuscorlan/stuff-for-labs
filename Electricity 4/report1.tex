...
current sources, the resistor will be connected in parallel\cite{alexander2017}. 

Figures 1(a) and 1(b) display the aforementioned representation of power
sources, both for current and voltage sources\cite{lab}.
\begin{figure}[!ht]
    \centering
...
sources with an internal resistance\cite{alexander2017}. Therefore, going forward, sources
...
voltage, and electromotive force\cite{lab}.
...
real voltage source such as the one shown in Figure 1(a)\cite{lab}.
...
internal resistance can be found\cite{lab}:
...
A relationship between all of the characteristics is found equation 3\cite{lab}:
...
parallel resistance equivalence\cite{alexander2017}:
...
efficiency of the circuit will only reach $50\%$.\cite{lab}
...
up to one significant figure, as instructed in the uncertainty analysis manual\cite{DeneyerUncertainty}. This value can be used to calculate the error of the internal
...
multimeter measurements\cite{multim}. This is done using equation 6.
\begin{equation}
...
\subsection{Discussion}
Firstly, by analyzing figures 11 and 12, one can observe that the power
dissipation does not exactly follow the expected parabolic shape. This is likely
due to the means of constructing the circuit, as the unseen means of connecting
the elements could add unexpected resistance. That being said, the maximal power
result is as expected, with it occurring when the load resistance is closest to
the internal resistance. 

It is important to mention that the third data point for the second source has
been excluded and considered an outlier. The unusual value is likely attributed
to an improper connection of either the voltmeter or the battery. Therefore, to
analyze the behaviour of power transfer, this data point has to be ignored.

\section{Conclusion}
This experiment was successful in determining the characteristics of DC power
sources. The analyzed characteristics were their internal resistances,
electromotive forces, power transfer efficiency. Different source arrangements
were also analyzed. 

For the internal resistances, it is found that the calculated internal
resistances corresponded to the measured values:
\begin{itemize}
    \item Source 1: $(61.5 \pm 0.1)\unit{\Omega}$ (calc.) vs. $(61.5 \pm
    0.8)\unit{\Omega}$ (measured)
    \item Source 2: $(147.4 \pm 0.1)\Omega$ (calc.) vs. $(148.2 \pm
    2)\unit{\Omega}$ (measured)
\end{itemize}
The theoretical predictions for the series ($R_{i,series} = 208.8\unit{\Omega}$)
and parallel ($R_{i,parallel} = 43.6\unit{\Omega}$) arrangements was also
validated.

$V_{emf}$ of the source arrangements also falls in accordance with the
theoretical background:
\begin{itemize}
    \item $V_{emf}$ for the parallel arrangement is equal to that of one source.
    \item $V_{emf}$ for the series arrangement is double that of one source.
\end{itemize}

For the maximum power transfer experiment, the peak power dissipation was
achieved when $R_L \approx R_i$. However, the graph deviated from the expected
parabolic relationship. This is attributed to the contact resistances in circuit
assembly.

All results adhered to the theoretical predictions made by the fundamental
circuit laws. However, certain deviations were observed. These are attributed to
both user error and systematic errors.
